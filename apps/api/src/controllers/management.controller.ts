...
export async function listPolicies(req:Request,res:Response){const query=z.object({page:z.coerce.number().int().positive().default(1),limit:z.coerce.number().int().min(1).max(100).default(50)}).parse(req.query);const [items,total]=await Promise.all([prisma.policy.findMany({orderBy:{createdAt:"desc"},skip:(query.page-1)*query.limit,take:query.limit}),prisma.policy.count()]);res.json({success:true,data:{items,total,page:query.page,limit:query.limit}});}
...
export async function listJeevan(_req:Request,res:Response){res.json({success:true,data:await prisma.jeevanPramaanRecord.findMany({select:{id:true,applicationNumber:true,status:true,submissionDate:true,verificationDate:true,remarks:true,createdAt:true,pensioner:{select:{employeeId:true,name:true,mobile:true}}},orderBy:{createdAt:"desc"},take:100})});}
